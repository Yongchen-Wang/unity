...
class DiscreteMazeEnv(gym.Env):
    ...
    def __init__(self, grid_map_path, original_image_path, max_episode_length=2e4, seed=0, render_mode='human'):
        ...
        # self.camera = cv2.VideoCapture(0)  # 打开摄像头，0表示默认摄像头
...
##### 只有强化学习
    ...
    def render(self, mode='human'):
        ...
        # ret, frame = self.camera.read()  # 获取摄像头图像
        # if not ret:
        ...
        # # 计算缩放比例（从 1800x1800 到 475x475）
        # scale_x = 600 / 1800
        # scale_y = 600 / 1800

        # # 根据缩放比例调整起点和终点的位置（交换了x和y轴）
        # start_pos_scaled = (int(self.start_pos[1] * scale_x), int(self.start_pos[0] * scale_y))
        # goal_pos_scaled = (int(self.goal_pos[1] * scale_x), int(self.goal_pos[0] * scale_y))
        ...
        # # 显示裁剪后的图像
        # cv2.imshow("Maze Environment", cropped_frame)
        # cv2.waitKey(1)  # 短暂停留，允许图像显示
...
#         self.camera_thread = threading.Thread(target=self._run_camera)
...
#         observation = np.concatenate((np.array(self.current_pos, dtype=np.int32), np.array(self.start_pos, dtype=np.int32), np.array(self.goal_pos, dtype=np.int32)))
#         done = (abs(self.current_pos[0] - self.goal_pos[0]) <= 30) and (abs(self.current_pos[1] - self.goal_pos[1]) <= 30)
#         if done:
#             print("done")
#             time.sleep(5)
...
#     def goto_position(self, position, speed=8000):
#         x, y = position
#         # 将图像网格坐标系映射到机器人的坐标系
#         y_robot = 18000 * x / 1800  # 图像网格的X轴对应机器人的Y轴
#         x_robot = 18000 * y / 1800  # 图像网格的Y轴对应机器人的X轴
...
#         y_robot = np.floor(y_robot).astype(int)
#         x_robot = np.floor(x_robot).astype(int)
#         print("x_robot,y_robot",x_robot,y_robot)
#         self.manipulator_left.goto_pos((x_robot, y_robot, 10000,10000), speed=speed)
#         # time.sleep(0.1)
...
#         cropped_frame = frame[2:478, 52:528]  # 半开区间 [start, end)
...
#     def close(self):
#         self.camera_thread_running = False  # 停止相机线程
#         if self.camera.isOpened():
#             self.camera.release()
#         cv2.destroyAllWindows()
